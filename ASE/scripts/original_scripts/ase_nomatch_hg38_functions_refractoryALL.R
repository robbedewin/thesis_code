...
get_alleles_chr_nomatch <- function(chr, allelesdir, countsdir, mindepth = 3, sample_id) {
  allelesfile <- file.path(allelesdir, paste0("1kg.phase3.v5a_GRCh38nounref_allele_index_chr", chr, ".txt"))
  ...
  alleles <- read_tsv(file = allelesfile, col_names = T, col_types = "iii")
  alleles$a0 <- c("A","C","G","T")[alleles$a0]
  alleles$a1 <- c("A","C","G","T")[alleles$a1]
  ...
  output <- alleles[!is.na(alleles$a0) & !is.na(alleles$a1) & alleles$count_ref >= mindepth & alleles$count_alt >= mindepth, ]
  write_tsv(x = cbind(chr = rep(chr, nrow(output)), output), file = outfile_allelecounts, col_names = F)
  return(NULL)
}
...
combine_loci_nomatch <- function(countsdir, sample_id, bsgnom) {
  ...
  write_tsv(x = allelecounts, file = outfile, col_names = T)
  ...
  allelecounts_vr <- VRanges(seqnames = paste0("chr", allelecounts$chr), ranges = IRanges(start = allelecounts$pos, end = allelecounts$pos), ref = allelecounts$ref, alt = allelecounts$alt) # , seqinfo = seqinfo(bsgnom)
  allelecounts_vr <- keepSeqlevels(x = allelecounts_vr, value = paste0("chr", c(1:22, "X")), pruning.mode = "coarse")
...
#   cmd <- paste0(JAVA, " -jar /srv/sw/eb/software/GATK/3.8-1-Java-1.8.0_221/GenomeAnalysisTK.jar",
#                 " -R ", refgenome,
#                 " -T ASEReadCounter",
#                 " -o ", outfile,
#                 " -I ", bamfile,
...
ase_annotate <- function(asedf, bsgnom, annot) {
  ...
  asegr <- GRanges(seqnames = paste0("chr", asedf$contig), ranges = IRanges(start = asedf$position, end = asedf$position)) #, seqinfo = seqinfo(bsgnom)
  # seqlevelsStyle(asegr) <- "UCSC"
  
  asegcoverlaps <- findOverlaps(query = asegr, subject = gc31)
  
  asedf$gene <- NA
  asedf$gene[unique(queryHits(asegcoverlaps))] <- by(data = gc31$gene_name[subjectHits(asegcoverlaps)], INDICES = queryHits(asegcoverlaps), FUN = paste0, collapse = ";")
...
plot_ase_manhattan <- function(asedf) {
  ...
  p1 <- ggplot(data = asedf, mapping = aes(x = 1:nrow(asedf), y = -log10(pval))) + geom_point(mapping = aes(colour = contig %in% as.character(seq(2,22,2))), show.legend = F, alpha = .3, size = .5) +
    geom_hline(yintercept = -log10(max(asedf[asedf$padj < .01, "pval"])), linetype = "dashed", colour = "grey") +
    geom_text(data = asedf[asedf$padj <= .01, ], mapping = aes(x = which(asedf$padj <= .01), y = -log10(pval), label = gene), size = 1.5, angle = 45, hjust = 0, nudge_x = nrow(asedf)/250, nudge_y = 0.1, alpha = .5, show.legend = F)
