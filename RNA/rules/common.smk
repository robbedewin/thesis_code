...
units.loc[units['datatype'] == 'rna', 'identifier_rna'] = units['sample_name']
...
# units.set_index('identifier', inplace=True)

# def get_fastq(wildcards):
#     identifier = f"{wildcards.sample}_{wildcards.datatype}_{wildcards.alias}"
#     try:
#         fastqs = units.loc[identifier]
#         return {"r1": fastqs["fq1"].strip(), "r2": fastqs["fq2"].strip()}
#     except KeyError:
#         raise ValueError(f"Missing FASTQ data for identifier: {identifier}")
...
# def get_fastq_atac(wildcards):
#     identifier = f"{wildcards.sample}_atac_{wildcards.alias}"
#     try:
#         fastqs = units.loc[identifier]
#         return {"r1": fastqs["fq1"].strip(), "r2": fastqs["fq2"].strip()}
#     except KeyError:
